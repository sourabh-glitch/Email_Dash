...
const AlertDashboard = () => {
  ...
  const STATUS_MAP = {
    ...
    4: 'Resolved',
    5: 'Closed',
    12: 'Service Restored Awaiting RCA/Fix',
    13: 'Issue Fixed - Under Monitoring',
  };

  useEffect(() => {
    const fetchAlertTickets = async () => {
      try {
        const response = await fetch(`${baseUrl}/api/tickets/Internal-Alert`);
        const data = await response.json();
        setTickets(data);
      } catch (error) {
        console.error('Error fetching Internal Alert tickets:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    fetchAlertTickets();
    const interval = setInterval(fetchAlertTickets, 10000);
...
const rowsPerPage = 10
  ...
  return (
    <div className="p-6 h-screen overflow-x-auto">
      <h2 className="text-2xl font-bold mb-4">Internal Alert Dashboard</h2>
      ...
      {loading ? (
        <p className="text-gray-600">Loading Internal Alert tickets...</p>
      ) : filteredTickets.length === 0 ? (
        <p className="text-gray-600">No Internal Alert tickets found matching your criteria.</p>
      ) : (
